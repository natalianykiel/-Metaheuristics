...
class Utils:
    ...
    def copy(self, init_pheromone_val):
        new_utils = copy.deepcopy(self)

        new_utils.init_pheromone_val = init_pheromone_val
        new_utils.pheromone_mat = np.ones((new_utils.node_num, new_utils.node_num)) * init_pheromone_val
    ...
    def create_from_file(self, file_path):
        ...
        for i in range(node_num):
            ...
            for j in range(i+1, node_num):
                node_b = nodes[j]
                node_dist_mat[i][j] = Utils.calculate_dist(node_a, node_b)
